#!/usr/bin/env python3
"""
IEEE Xplore Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search IEEE-published papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with IEEE publisher filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, IEEE papers mostly paywalled)
6. Build Excel
"""
...
# ============================================================
# IEEE PUBLISHER ID in OpenAlex
# ============================================================
IEEE_PUBLISHER_ID = 'p4310319808'
...
def search_openalex_acm(query, per_page=200, max_results=2000):
    """Search OpenAlex for IEEE papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
    ...
    while cursor and fetched < max_results:
        params = urllib.parse.urlencode({
            'search': query,
            'filter': f'primary_location.source.publisher_lineage:{IEEE_PUBLISHER_ID}',
            'per_page': per_page,
            'cursor': cursor,
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for IEEE)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For IEEE papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='003366', end_color='003366', fill_type='solid')  # Dark blue for IEEE
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: IEEE Xplore Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'IEEE Xplore (via OpenAlex API)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to IEEE publisher'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    out = '/home/user/Bias-Review-Paper/IEEE_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("IEEE Xplore Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching IEEE via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_acm(q['search'])
        new_count = 0
    ...
    print(f"\n  Total unique IEEE papers: {len(all_papers)}")
    ...
    # Step 3: Full-text screening (abstract-based for IEEE)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/ieee_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: ieee_screening_data.json")

    # Results
    print(f"\n{'='*70}")
    print("IEEE SCREENING RESULTS")
    print(f"{'='*70}")
