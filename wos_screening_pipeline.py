#!/usr/bin/env python3
"""
Web of Science Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search Web of Science papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with Web of Science filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, Web of Science papers mostly paywalled)
6. Build Excel
"""
...
# ============================================================
# WEB OF SCIENCE FILTER ID in OpenAlex
# ============================================================
FILTER_ID = 'p4310319965'
...
def search_openalex_wos(query, per_page=200, max_results=2000):
    """Search OpenAlex for Web of Science papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for Web of Science)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For Web of Science papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='2D5F8A', end_color='2D5F8A', fill_type='solid')  # Web of Science (Springer Nature)
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='EBF2F8', end_color='EBF2F8', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: Web of Science Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'Web of Science (via Springer Nature publisher filter in OpenAlex)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to Springer Nature publisher'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='2D5F8A')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='2D5F8A')
    ...
    out = '/home/user/Bias-Review-Paper/Web_of_Science_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("Web of Science Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching Web of Science via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_wos(q['search'])
        new_count = 0
    ...
    print(f"\n  Total unique Web of Science papers: {len(all_papers)}")
    ...
    # Step 3: Full-text screening (abstract-based for Web of Science)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/wos_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: wos_screening_data.json")

    # Results
    print(f"\n{'='*70}")
    print("WEB OF SCIENCE SCREENING RESULTS")
    print(f"{'='*70}")
