#!/usr/bin/env python3
"""
PubMed Step 4: Generate 2 Final Excel Files

File 1: PubMed_FullText_Screened.xlsx — 424 papers that passed both stages
File 2: PubMed_NoFullText.xlsx — 366 papers that passed T/A only (no full text available)

Both files include:
- Summary sheet with full pipeline numbers and criteria
- Papers sheet with extracted columns
...
def build_excel(papers, config, pipeline_data):
    ...
    rows = [
        ...
        ['Date:', '2026-02-18'],
        ['Total queries:', f"{len(p['query_stats'])} (7 original + 10 supplementary)"],
        [''],
        ['SCREENING FLOW', ''],
        ['Total unique PMIDs found:', str(p['total_unique_pmids'])],
        ['Successfully fetched:', str(p['total_fetched'])],
    ...
    rows.append(['  Fetch failed:', str(ft['ft_fetch_failed'])])
    ...
    qh = [('ID', 5), ('Label', 40), ('Query', 80), ('Total in PubMed', 15), ('New Unique', 12), ('Cumulative', 12)]
    ...
    for idx, qs in enumerate(p['query_stats'], 1):
        vals = [qs['id'], qs['label'], qs['query'],
                qs.get('total_in_pubmed', ''), qs.get('new_unique', ''),
                qs.get('cumulative', qs.get('cumulative_new', ''))]
        for ci, v in enumerate(vals, 1):
...
def main():
    print("=" * 70)
    print("STEP 4: Generate Final Excel Files")
    print("=" * 70)

    # Load Step 3 results
    with open(f'{BASE_DIR}/pubmed_step3_results.json') as f:
        data = json.load(f)

    ft_included = data['ft_included']
    no_fulltext = data['no_fulltext']

    # Extract columns for all papers
