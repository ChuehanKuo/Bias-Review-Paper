#!/usr/bin/env python3
"""
Scopus Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search Scopus papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with Scopus filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, Scopus papers mostly paywalled)
6. Build Excel
"""
...
# ============================================================
# SCOPUS FILTER ID in OpenAlex
# ============================================================
FILTER_ID = 'p4310320990'
...
def search_openalex_scopus(query, per_page=200, max_results=2000):
    """Search OpenAlex for Scopus papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for Scopus)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For Scopus papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='E87722', end_color='E87722', fill_type='solid')  # Scopus (Elsevier)
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='FFF3E8', end_color='FFF3E8', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: Scopus Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'Scopus (via Elsevier publisher filter in OpenAlex)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to Elsevier BV publisher (Scopus parent company)'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='E87722')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='E87722')
    ...
    out = '/home/user/Bias-Review-Paper/Scopus_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("Scopus Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching Scopus via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_scopus(q['search'])
        new_count = 0
    ...
    # Step 3: Full-text screening (abstract-based for Scopus)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/scopus_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: scopus_screening_data.json")
