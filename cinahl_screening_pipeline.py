#!/usr/bin/env python3
"""
CINAHL Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search CINAHL papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with CINAHL filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, CINAHL papers mostly paywalled)
6. Build Excel
"""
...
# ============================================================
# CINAHL FILTER ID in OpenAlex
# ============================================================
FILTER_ID = 'p4310320017'
...
def search_openalex_cinahl(query, per_page=200, max_results=2000):
    """Search OpenAlex for CINAHL papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for CINAHL)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For CINAHL papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='7B2D8E', end_color='7B2D8E', fill_type='solid')  # CINAHL (SAGE)
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='F5EBF8', end_color='F5EBF8', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: CINAHL Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'CINAHL (via SAGE Publications publisher filter in OpenAlex)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to SAGE Publications publisher (strong nursing/allied health coverage)'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='7B2D8E')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='7B2D8E')
    ...
    out = '/home/user/Bias-Review-Paper/CINAHL_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("CINAHL Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching CINAHL via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_cinahl(q['search'])
        new_count = 0
    ...
    print(f"\n  Total unique CINAHL papers: {len(all_papers)}")
    ...
    # Step 3: Full-text screening (abstract-based for CINAHL)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/cinahl_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: cinahl_screening_data.json")

    # Results
    print(f"\n{'='*70}")
    print("CINAHL SCREENING RESULTS")
    print(f"{'='*70}")
