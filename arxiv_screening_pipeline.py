...
# ============================================================
# ARXIV FILTER ID in OpenAlex
# ============================================================
FILTER_ID = 'S4306400194'
...
def search_openalex_arxiv(query, per_page=200, max_results=2000):
    """Search OpenAlex for arXiv papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
    ...
    while cursor and fetched < max_results:
        params = urllib.parse.urlencode({
            'search': query,
            'filter': f'primary_location.source.id:{FILTER_ID}',
            'per_page': per_page,
            'cursor': cursor,
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for arXiv)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For arXiv papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='B31B1B', end_color='B31B1B', fill_type='solid')  # arXiv
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='FCEAEA', end_color='FCEAEA', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: arXiv Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'arXiv (via source filter in OpenAlex)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to arXiv repository source'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='B31B1B')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='B31B1B')
    ...
    out = '/home/user/Bias-Review-Paper/arXiv_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("arXiv Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching arXiv via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_arxiv(q['search'])
        new_count = 0
    ...
    print(f"\n  Total unique arXiv papers: {len(all_papers)}")
    ...
    # Step 3: Full-text screening (abstract-based for arXiv)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/arxiv_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: arxiv_screening_data.json")

    # Results
    print(f"\n{'='*70}")
    print("ARXIV SCREENING RESULTS")
    print(f"{'='*70}")
