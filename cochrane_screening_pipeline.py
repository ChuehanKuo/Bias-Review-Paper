#!/usr/bin/env python3
"""
Cochrane Library Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search Cochrane Library papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with Cochrane Library filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, Cochrane Library papers mostly paywalled)
6. Build Excel
"""
...
# ============================================================
# COCHRANE LIBRARY FILTER ID in OpenAlex
# ============================================================
FILTER_ID = 'p4310320595'
...
def search_openalex_cochrane(query, per_page=200, max_results=2000):
    """Search OpenAlex for Cochrane Library papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for Cochrane Library)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For Cochrane Library papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='00457C', end_color='00457C', fill_type='solid')  # Cochrane (Wiley)
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='E6F0FA', end_color='E6F0FA', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: Cochrane Library Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'Cochrane Library (via Wiley publisher filter in OpenAlex)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to Wiley publisher (Cochrane Library publisher)'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='00457C')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='00457C')
    ...
    out = '/home/user/Bias-Review-Paper/Cochrane_Library_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("Cochrane Library Screening Pipeline")
    print("Systematic Review: Algorithmic Bias in Health AI")
    print("=" * 70)

    # Step 1: Search
    print(f"\nSTEP 1: Searching Cochrane Library via OpenAlex ({len(QUERIES)} queries)...")
    all_papers = {}
    query_stats = []

    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex_cochrane(q['search'])
        new_count = 0
    ...
    print(f"\n  Total unique Cochrane Library papers: {len(all_papers)}")
    ...
    # Step 3: Full-text screening (abstract-based for Cochrane Library)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/cochrane_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: cochrane_screening_data.json")

    # Results
    print(f"\n{'='*70}")
    print("COCHRANE LIBRARY SCREENING RESULTS")
    print(f"{'='*70}")
