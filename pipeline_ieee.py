#!/usr/bin/env python3
"""
IEEE Xplore Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API with IEEE publisher filter.
1. Search OpenAlex (IEEE publisher)
2. Deduplicate
...
def main():
    print("=" * 70)
    print("IEEE Xplore Pipeline — Systematic Review (Fresh Run)")
    print("=" * 70)
    ...
    for q in QUERIES:
        print(f"\n  {q['id']}: {q['label']}")
        papers, count = search_openalex(q['search'], IEEE_PUBLISHER_ID)
        new_count = 0
    ...
    # Step 3: Full-text screening via PMC (DOI lookup)
    print(f"\nSTEP 3: Full-text screening via PMC for {len(ta_included)} papers...")
    included, ft_excluded, ft_stats = ss.run_fulltext_screening(ta_included, id_field='doi', db_label='IEEE')
    ...
    config = {
        'db_name': 'IEEE Xplore (via OpenAlex)',
        'db_label': 'IEEE',
        'header_color': '003366',
        'alt_row_color': 'E8F0FE',
        'id_field': 'doi',
        'id_label': 'DOI',
        'output_path': f'{BASE_DIR}/IEEE_Screening_Results.xlsx',
        'date': '2026-02-17',
        'search_desc': 'OpenAlex API with IEEE publisher filter (p4310319808)',
    }
    ss.build_excel(included, all_excluded, query_stats, meta, config)

    # Save JSON
    json_path = f'{BASE_DIR}/ieee_screening_data.json'
