#!/usr/bin/env python3
"""
ACM Digital Library Screening Pipeline for Systematic Review:
"Approaches for Assessing and Mitigating Algorithmic Bias in Health AI"

Uses OpenAlex API to search ACM-published papers.
Same screening criteria and column extraction as PubMed pipeline.

1. Search OpenAlex with ACM publisher filter
2. Deduplicate
3. Screen by title+abstract
4. Extract structured columns
5. Full-text screen (abstract-based, ACM papers mostly paywalled)
6. Build Excel
"""
...
def search_openalex_acm(query, per_page=200, max_results=2000):
    """Search OpenAlex for ACM papers matching query."""
    results = {}
    cursor = '*'
    fetched = 0
    ...
    while cursor and fetched < max_results:
        params = urllib.parse.urlencode({
            'search': query,
            'filter': f'primary_location.source.publisher_lineage:{ACM_PUBLISHER_ID}',
            'per_page': per_page,
            'cursor': cursor,
...
# ============================================================
# FULL-TEXT SCREENING (abstract-based for ACM)
# ============================================================

def fulltext_screen(paper):
    """
    Full-text screening with strict criteria.
    For ACM papers, we use abstract only since most are paywalled.
    """
    title = (paper.get('title', '') or '').lower()
    abstract = (paper.get('abstract', '') or '').lower()
    combined = title + ' ' + abstract
...
H_FILL = PatternFill(start_color='8B4513', end_color='8B4513', fill_type='solid')  # Brown for ACM
H_ALIGN = Alignment(horizontal='center', vertical='center', wrap_text=True)
C_ALIGN = Alignment(vertical='top', wrap_text=True)
BORDER = Border(left=Side(style='thin'), right=Side(style='thin'),
                top=Side(style='thin'), bottom=Side(style='thin'))
ALT_FILL = PatternFill(start_color='FFF5EE', end_color='FFF5EE', fill_type='solid')
...
def build_excel(included, excluded, query_stats, meta):
    ...
    rows = [
        ['SYSTEMATIC REVIEW: ACM Digital Library Screening Results'],
        [''],
        ['Review Title:', 'Approaches for Assessing and Mitigating Algorithmic Bias in Health AI'],
        ['Database:', 'ACM Digital Library (via OpenAlex API)'],
        ['Date:', '2026-02-16'],
        ['Queries:', f'{len(query_stats)} focused text searches filtered to ACM publisher'],
        [''],
        ['SCREENING FLOW:', ''],
    ...
    for ri, rd in enumerate(rows, 1):
        for ci, v in enumerate(rd, 1):
            ...
            if ri == 1:
                cell.font = Font(bold=True, size=14, color='8B4513')
            elif v in ['  INCLUDED:']:
                ...
                cell.font = Font(bold=True, size=12, color='8B4513')
    ...
    out = '/home/user/Bias-Review-Paper/ACM_Screening_Results.xlsx'
    wb.save(out)
    print(f"\nSaved: {out}")
...
def main():
    print("=" * 70)
    print("ACM Digital Library Screening Pipeline")
    ...
    # Step 3: Full-text screening (abstract-based for ACM)
    ...
    # Save JSON backup
    with open('/home/user/Bias-Review-Paper/acm_screening_data.json', 'w') as f:
        json.dump({
    ...
    print("Saved JSON backup: acm_screening_data.json")
